;( function( $, window, document, undefined ) {
	...
	$.extend( Plugin.prototype, {
		init: function() {
			...
			// cache the steps
			this.$steps = $('.iwd_opc_column_name');

			// get the initial acc-step height so we can calculate offset in animation
			this.stepHeight = $('.iwd_opc_column_name').eq(0).outerHeight();
			...
			if( this.settings.autoButtons ) {
				this.$steps.each(function(i, el) {
					...
					var $content = $('.iwd_opc_column_content', el);
		...
		activateStep: function(index, disableScrollOverride) {
	    	...
	    	$('.iwd_opc_column_content', this.element).slideUp();

			this.$steps.eq(index)
				.addClass('open')
				.find('.iwd_opc_column_content').slideDown();
		...
		activateAllSteps: function() {
			...
			$('.iwd_opc_column_content', this.element).show();
		}
	});
